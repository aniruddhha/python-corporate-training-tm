...
('Smart Doorbell', 149.99, 'Smart Home Tech', 35),
...
select max(quantity) as max_qty, avg(quantity) as avg_qty, min(quantity)
from products;

-- i want to find Max price of the product which is purchased from perticular supplier

select max(price)
from products
where supplier = 'Lighting Solutions';

-- Group By

select supplier, sum(price) as ttl
from products
group by supplier
order by ttl desc;
