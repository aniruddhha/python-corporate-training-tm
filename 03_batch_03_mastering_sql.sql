...
select name 
from products
where quantity >= 1 and supplier = 'Urban Transport';

select name 
from products
where supplier = 'Urban Transport';
...
select max(quantity)
from products
where supplier = 'Appliance World';

-- group by
select supplier, sum(quantity), sum(price) as ttl_pr
from products
group by supplier
order by ttl_pr desc;
